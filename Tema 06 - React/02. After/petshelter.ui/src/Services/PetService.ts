...
export class PetService {
  ...
  addPetToDatabase(pet: PetDTO) {
    return axios.post("https://localhost:7075/Pets", pet);
  }
  ...
  validatePet(pet:PetDTO):Validator{
    const validator:Validator= {isCorrect:false}
    if (pet.name.length < 3) {
        validator.errorMessage='Name length must be >2';
        return validator;
    }
    if (pet.rescuer.name.length < 3) {
        validator.errorMessage='Name length must be >2';
        return validator;
    }
    if (pet.description.length < 3) {
        validator.errorMessage='Description length must be >3';
        return validator;
    }
    if (pet.weightInKg <= 0) {
        validator.errorMessage='Pet weight must be above 0'
        return validator;
    }
    if (pet.rescuer.idNumber.length !== 13) {
        validator.errorMessage='Id number must be 13 characters in length';
        return validator;
    }
